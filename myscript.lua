...
-- ===== ORCA HUB (ОТКРЫВАЕТСЯ КЛАВИШЕЙ K) =====
local function toggleOrca()
    print("🐚 Открываем Orca Hub (клавиша K)")
...
-- ===== INFINITE YIELD (ОТКРЫВАЕТСЯ КЛАВИШЕЙ INSERT ИЛИ /) =====
local function toggleIY()
    print("⚡ Открываем Infinite Yield (клавиша Insert или /)")
    -- Пробуем разные клавиши для Infinite Yield
    pressKey(Enum.KeyCode.Insert)
    task.wait(0.1)
    pressKey(Enum.KeyCode.Slash)
...
-- ===== SP HUB (ОТКРЫВАЕТСЯ КЛАВИШЕЙ K) =====
local function toggleSP()
    print("🎮 Открываем SP Hub (клавиша K)")
    pressKey(Enum.KeyCode.K)
...
if orcaLoaded then
    print("✅ Orca Hub загружен! Нажмите кнопку 🐚 Orca")
else
...
if spLoaded then
    print("✅ SP Hub загружен! Нажмите кнопку 🎮 SP")
else
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    elseif input.KeyCode == Enum.KeyCode.K then orcaButton:TriggerClick()
